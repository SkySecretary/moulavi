...
export default function PartyMissingBRNPage() {
  ...
  const fetchBookings = useCallback(async (page = 1) => {
    try {
      ...
      const partyBookings = allBookings.filter((b: any) => b.partyId === user?.partyId);
  ...
  const fetchHistory = useCallback(async (page = 1) => {
    try {
      ...
      // Filter history logs for party's bookings
      const allLogs = response.data.history || [];
      const partyLogs = allLogs.filter((l: any) => l.booking?.party?.id === user?.partyId || l.booking?.partyId === user?.partyId);
      
      setHistoryLogs(partyLogs);
      setHistoryPagination({
        ...response.data.pagination,
        total: partyLogs.length,
        totalPages: Math.ceil(partyLogs.length / 50)
      });
    } catch (error) {
